...
hold on;
for i=1:length(lambda)
  stem(k,poisson(i,:),"linewidth",1.2);
end
hold off;
...
mean_value = 0;
for i=0:(length(poisson(index,:))-1)
...
figure(2);
hold on;
stem(k,poisson_first(:),"linewidth",1.2);
stem(k,poisson_second(:),"linewidth",1.2);
